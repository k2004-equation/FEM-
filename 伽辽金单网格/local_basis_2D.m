function result=local_basis_2D(x,y,vertices,basis_type,basis_index,basis_der_x,basis_der_y)

xn1=vertices(1,1);
xn2=vertices(1,2);
xn3=vertices(1,3);
yn1=vertices(2,1);
yn2=vertices(2,2);
yn3=vertices(2,3);

J=(xn2-xn1)*(yn3-yn1)-(xn3-xn1)*(yn2-yn1);

xh=((yn3-yn1)*(x-xn1)-(xn3-xn1)*(y-yn1))/J;
yh=(-(yn2-yn1)*(x-xn1)+(xn2-xn1)*(y-yn1))/J;

if basis_der_x==0 && basis_der_y==0

    result=reference_basis_2D(xh,yh,basis_type,basis_index,0,0);

elseif basis_der_x==1 && basis_der_y==0

    der_x_result=reference_basis_2D(xh,yh,basis_type,basis_index,1,0);
    der_y_result=reference_basis_2D(xh,yh,basis_type,basis_index,0,1);

    result=der_x_result*(yn3-yn1)/J+der_y_result*(yn1-yn2)/J;
  

elseif basis_der_x==0 && basis_der_y==1

    der_x_result=reference_basis_2D(xh,yh,basis_type,basis_index,1,0);
    der_y_result=reference_basis_2D(xh,yh,basis_type,basis_index,0,1);

    result=der_x_result*(xn1-xn3)/J+der_y_result*(xn2-xn1)/J;
 

elseif basis_der_x==1 && basis_der_y==1

    der_xx_result=reference_basis_2D(xh,yh,basis_type,basis_index,2,0);
    der_xy_result=reference_basis_2D(xh,yh,basis_type,basis_index,1,1);
    der_yx_result=reference_basis_2D(xh,yh,basis_type,basis_index,1,1);
    der_yy_result=reference_basis_2D(xh,yh,basis_type,basis_index,0,2);

    result=der_xx_result*(xn1-xn3)*(yn3-yn1)/J^2+der_xy_result*(xn1-xn3)*(yn1-yn2)/J^2+....
    der_yx_result*(xn2-xn1)*(yn3-yn1)/J^2+der_yy_result*(xn2-xn1)*(yn1-yn2)/J^2;

elseif basis_der_x==2 && basis_der_y==0

    der_xx_result=reference_basis_2D(xh,yh,basis_type,basis_index,2,0);
    der_xy_result=reference_basis_2D(xh,yh,basis_type,basis_index,1,1);
    der_yy_result=reference_basis_2D(xh,yh,basis_type,basis_index,0,2);

    result=der_xx_result*(yn3-yn1)^2/J^2+2*der_xy_result*(yn1-yn2)*(yn3-yn1)/J^2+....
    der_yy_result*(yn1-yn2)^2/J^2;

elseif basis_der_x==0 && basis_der_y==2

    der_xx_result=reference_basis_2D(xh,yh,basis_type,basis_index,2,0);
    der_xy_result=reference_basis_2D(xh,yh,basis_type,basis_index,1,1);
    der_yy_result=reference_basis_2D(xh,yh,basis_type,basis_index,0,2);

    result=der_xx_result*(xn1-xn3)^2/J^2+2*der_xy_result*(xn1-xn3)*(xn2-xn1)/J^2+....
    der_yy_result*(xn2-xn1)^2/J^2;

end
