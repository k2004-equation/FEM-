...
%------------无穷范数------L2范数------H1范数-------时间成本
%2×2 4×4      0.0023    0.0010     0.0110
%4×4 16×16    0.0008     0.0002    0.0102
%8×8 64×64
...
XmH(:,1)=inital_vector_u(P_trial(1,:)',P_trial(2,:)');
%初值条件（当前时间解向量）
...
for n=1:Nt
    ...
    while max(abs(dXL))>1e-9
        ...
        trial_basis_der_A_x = 0;trial_basis_der_A_y = 0;
        test_basis_der_A_x  = 0;test_basis_der_A_y  = 0;
        At = assemble_matrix_2D_ns_ns(matrix_size_A, gauss_point, gauss_weight, 'function_feval_c1','function_gudu',XL,XmH(:,n), P, T, T_trial, T_test, trial_basis_type_c, trial_basis_der_A_x_p,trial_basis_der_A_y_p,trial_basis_type_c, trial_basis_der_A_x_q,trial_basis_der_A_y_q,trial_basis_type, trial_basis_der_A_x,trial_basis_der_A_y, test_basis_type, test_basis_der_A_x,test_basis_der_A_y);    
        ...
        trial_basis_der_b_x = 0;trial_basis_der_b_y = 0;
        test_basis_der_b_x = 0;test_basis_der_b_y = 0;
        bt = assemble_vector_2D_ns_ns(vector_size_b, gauss_point, gauss_weight, 'function_feval_c1','function_gu', XL,XmH(:,n),trial_basis_type, trial_basis_der_b_x,trial_basis_der_b_y,P, T,P_trial,T_trial, T_test, test_basis_type, test_basis_der_b_x,test_basis_der_b_y);
...
for n=1:Nt
    ...
    trial_basis_der_b_x = 1;trial_basis_der_b_y = 0;
    test_basis_der_b_x = 1;test_basis_der_b_y = 0;%向量b中测试函数的导数阶数
...
error_H1_u(n)=sqrt(error_H1_u(n));
end

error=[error_max_u(Nt+1);error_L2_u(Nt+1);error_H1_u(Nt+1)]
