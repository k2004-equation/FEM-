...
gauss_type=9;%n点高斯公式，可供使用3,4,9点（至少满足二次多项式精度要求）

trial_basis_type=202;%试探函数u的基函数类型
test_basis_type=202;%测试函数v的基函数类型
%202表示二维二次rectangular,只能用这个
%试探函数p和测试函数q都是201，直接使用P、T


% 未收敛
%单元数2×2,  二维maxerror_u=0.0080---------maxerror_p=2.1364
%单元数4×4,  二维maxerror_u=0.0063---------maxerror_p=3.1212
%单元数8×8,  二维maxerror_u=0.0070---------maxerror_p=6.0445
%单元数16×16,二维maxerror_u=0.0123---------maxerror_p=12.5427
%单元数32×32,二维maxerror_u=------maxerror_p=
%单元数64×64,二维maxerror_u=------maxerror_p=
%误差阶--------------4次（十六倍）---------------2次（四倍）
...
%以下是二维Navier-Stokes方程问题的求解器（采用矩形单元）
% -------------------------------------
%区分网格概念和有限元概念，在线性元下网格剖分跟有限元是相同的，故默认P，T是201的
basis_type=201;
[P,T] = rectangular_generate_PbTb(num_of_element_r,num_of_element_c,interval_r,interval_c,basis_type);
%P是所有单元网格节点的坐标矩阵，T是所有单元上网格节点的全局编号矩阵

[P_trial,T_trial] = rectangular_generate_PbTb(num_of_element_r,num_of_element_c,interval_r,interval_c, trial_basis_type);
[P_test,  T_test] = rectangular_generate_PbTb(num_of_element_r,num_of_element_c,interval_r,interval_c,  test_basis_type);
...
[gauss_weight,gauss_point]=generate_Gauss_reference(gauss_type);
%根据确定的高斯积分公式提取相应的高斯节点和权重（注意参考单元为（0,1）高斯节点和权重）
